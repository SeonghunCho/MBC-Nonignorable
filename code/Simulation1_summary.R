...
for(i1 in 1:length(method_vec)){
  for(i2 in 1:length(PS_vec)){
    load(file=paste0(wd,"output/res_sim1/res_",i1,i2,".RData"))
...
tt <- NULL
for(i2 in 1:length(PS_vec)){
  tt_PS <- cbind(c(paste0("\\multirow{",nmethod,"}{*}{RM",i2,"}"),rep("",nmethod-1)),method_vec)
  for(pn in pn_vec){
    RES_tmp <- RES_total %>%
      filter(PS==i2,parname==pn) %>%
...
cat(tt,file=paste0(wd,"output/summary/sim1_table.txt"))
##############################################################################
## Figure 1
...
title_vec <- paste0("(",c("a","b","c","d"),") RM",PS_vec)
bp_total <- list()
for(i2 in 1:length(PS_vec)){
  RES_tmp <- RES_total %>%
    filter(PS==PS_vec[i2],parname%in%pn_vec) %>%
    mutate(method = factor(method,levels=method_vec)) %>%
    mutate(bias = thetahat-theta) %>%
...
ggsave(filename=paste0(wd,"output/summary/sim1_boxplot.pdf"),
